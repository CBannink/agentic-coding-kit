...
# ------------------------------------------------------------------
# Phase 3.5 -- reusables + workflow briefs
# ------------------------------------------------------------------
echo "kit-bootstrap: Phase 3.5 -- reusables + workflow briefs..." >&2

_reusables_file="$REPO/.kit/context/reusables.md"
_brief_dir="$REPO/.kit/context/workflow-briefs"
mkdir -p "$_brief_dir"

_src_hint="No stable source directory detected yet."
if [ -d "$REPO/src" ]; then
    _src_hint="src/ contains primary source files."
elif [ -d "$REPO/lib" ]; then
    _src_hint="lib/ contains primary source files."
elif [ -d "$REPO/app" ]; then
    _src_hint="app/ contains primary source files."
fi

_test_hint="No stable test directory detected yet."
if [ -d "$REPO/tests" ]; then
    _test_hint="tests/ contains test files."
elif [ -d "$REPO/test" ]; then
    _test_hint="test/ contains test files."
fi

_first_source="$(find "$REPO" -maxdepth 3 -type f \( -name '*.py' -o -name '*.ts' -o -name '*.tsx' -o -name '*.js' -o -name '*.jsx' \) 2>/dev/null | sed "s#^$REPO/##" | head -1 || true)"
_first_test="$(find "$REPO" -maxdepth 3 -type f \( -name 'test_*.py' -o -name '*.test.ts' -o -name '*.spec.ts' -o -name '*.test.tsx' -o -name '*.spec.tsx' -o -name '*.test.js' \) 2>/dev/null | sed "s#^$REPO/##" | head -1 || true)"
[ -n "$_first_source" ] || _first_source="README.md"
[ -n "$_first_test" ] || _first_test="No test file detected"

cat > "$_reusables_file" << REUSABLES_EOF
# Reusables Index

Generated by kit-bootstrap on $_today. Keep this under 5000 tokens.

## API and data layer
- No stable reusable API/data helpers detected yet.

## UI/design system
- No stable UI component system detected yet.

## State and stores
- No stable state/store layer detected yet.

## Cross-cutting utilities
- No stable shared utility module detected yet.

## Domain interfaces/types
- $_first_source - inspect before introducing new domain objects.

## Source
- Generated from shell inspection of: $_top_files
- Re-run /bootstrap-harness after the repo has more code to replace this compact starter index.
REUSABLES_EOF
...
Generated by kit-bootstrap on $_today. Keep under 5000 tokens.

## Repo snapshot
- Repo: $_repo_name
- Source layout: $_src_hint
- Tests: $_test_hint
- First source evidence: $_first_source:1
- First test evidence: $_first_test

## Context to read first
- .kit/context/conventions.md - repo workflow and architecture preferences.
- .kit/context/memory.md - durable repo facts.
- .kit/context/reusables.md - reusable APIs/components/utilities.
- .wiki/index.md - wiki entry point before deeper docs.
- .wiki/codebase.md and .wiki/architecture.md - placement and boundaries.

## Role guidance
$role_note

## Placeholder status
- This is real bootstrap output, not a template placeholder.
- If facts are still thin, say so and prefer current code over assumptions.
BRIEF_EOF
}

_write_brief "$_brief_dir/workflow-explorer.md" "Workflow Explorer Brief" \
"- Map files and contracts before implementation.
- Distinguish evidence from inference.
- Prefer small targeted searches over broad repo reads."

_write_brief "$_brief_dir/workflow-implementer.md" "Workflow Implementer Brief" \
"- Read this brief before editing.
- Reuse entries from .kit/context/reusables.md before creating new helpers.
- Follow .kit/context/conventions.md for file placement, tests, and error handling.
- Read touched files and nearest tests before writing."

_write_brief "$_brief_dir/workflow-reviewer.md" "Workflow Reviewer Brief" \
"- Review diffs against conventions.md, memory.md, and nearest tests.
- Prioritize correctness, regressions, missing verification, and repo convention drift.
- Avoid style-only findings unless they affect maintainability."

_write_brief "$_brief_dir/workflow-skeptic.md" "Workflow Skeptic Brief" \
"- Pressure-test plans and diffs for hidden failure modes.
- Challenge assumptions about file placement, integration points, and verification.
- Mark risks as evidence-backed or inferred."

_write_brief "$_brief_dir/workflow-ui-qa.md" "Workflow UI QA Brief" \
"- Use .wiki/features.md as the user-visible behavior contract.
- Check empty/loading/error states when UI exists.
- If no UI exists yet, report that UI-specific guidance is not applicable."

_write_brief "$_brief_dir/prompt-synthesizer.md" "Prompt Synthesizer Brief" \
"- Compress noisy handoffs into a small worker prompt.
- Preserve hard constraints, target files, verification commands, and repo conventions.
- Do not invent missing facts; label assumptions clearly."

echo "kit-bootstrap: Phase 3.5 done." >&2
...
for f in \
    ...
    "$REPO/.kit/context/reusables.md" \
    "$REPO/.kit/context/workflow-briefs/workflow-explorer.md" \
    "$REPO/.kit/context/workflow-briefs/workflow-implementer.md" \
    "$REPO/.kit/context/workflow-briefs/workflow-reviewer.md" \
    "$REPO/.kit/context/workflow-briefs/workflow-skeptic.md" \
    "$REPO/.kit/context/workflow-briefs/workflow-ui-qa.md" \
    "$REPO/.kit/context/workflow-briefs/prompt-synthesizer.md" \
    "$REPO/.wiki/index.md" \
