...
1. Check if `bash ~/.agents/bin/copilot/kit-bootstrap.sh` exists. If yes, run it:
   ```bash
   bash ~/.agents/bin/copilot/kit-bootstrap.sh "$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
   ```
   This runs the full chain: git-archaeology → convention detection → kit-init → wiki-init.
