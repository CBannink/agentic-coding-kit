...
hand_off.md, memory/MEMORY.md, project-scoped host agent dirs, etc.) to
coexist with the kit. Idempotent + non-destructive: archives, never deletes.

Read and follow `__SKILL_ROOT__/kit-migrate/SKILL.md` exactly:
...
Hard rules: idempotent, evidence-grounded, repo wins on project-scoped host
agent/command/settings dirs, never `rm` user data, ask before destructive operations.
