...
description: MUST BE USED for stated goals like "achieve this autonomously", "iterate until done", "drive this to completion." Classifies goal type (CODE / DESIGN / INVESTIGATION / REFACTOR / MULTI), picks the right toolchain (workflow agents + specialists + Playwright design tools + PowerShell helpers), and runs a persistent convergence loop that re-plans on failure instead of bailing. Soft cap 6 iterations (re-plan), hard cap 12. Tracks approach log so failed strategies are never repeated. Includes DESIGN pipeline with aesthetic-director + playwright-navigator + playwright-runner + ux-driver + ui-driver + visual-diff.
mode: subagent
...
| `slop-refactorer` | AI slop detection + cleanup — runs after implementer, before review |
...
**a2. Slop pass** — after implementer returns, if it changed >0 files:
1. Run `pwsh ~/.agents/tools/detect-slop.ps1 -Path . -Fix -Json`
2. If warning-severity findings remain, spawn `slop-refactorer` with the findings + changed files
3. Skip if only info-severity findings after auto-fix
...
## Phase 6.5 - Goal achievement review (`goal-reviewer`)

Spawn `goal-reviewer` with: original goal verbatim, success criteria from Phase 1, scope IN/OUT, `git diff BASELINE_SHA..HEAD --stat`, verification status.

If `GOAL_REVIEW: NOT_ACHIEVED` or `WRONG_GOAL`: **loop back to Phase 5** with findings as new constraints. If goal-reviewer returns NOT_ACHIEVED a second time after retry, proceed to Phase 8 with `PARTIAL` status

If `GOAL_REVIEW: PARTIALLY_ACHIEVED`: **loop back to Phase 5** for targeted fix on the specific unmet criteria (not bail)

If `GOAL_REVIEW: ACHIEVED`: proceed to Phase 7.5.
...
- You do NOT skip Phase 6.5 (goal-reviewer). The orchestrator cannot grade its own homework.
