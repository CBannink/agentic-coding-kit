...
| `SESSION-ONLY` | `${AGENTS_SESSION_ROOT}/{session_id}/handoffs.md` or `scratch.md` (default `.kit/session-state/{session_id}/...` in a bootstrapped repo, else `~/.agents/session-state/{session_id}/...`) | Task progress, one-off notes, speculative ideas, unresolved local context |
...
`${AGENTS_SESSION_ROOT}/{session_id}/workflow-evidence.json`
...
`${AGENTS_SESSION_ROOT}/{session_id}/run-packet.json`
