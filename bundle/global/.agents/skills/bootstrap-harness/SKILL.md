...
| `.kit/context/workflow-briefs/*.md` | Compact role-specific briefs for core workflow agents; each under 5000 tokens |
...
### Phase 4.6 -- Write Workflow Briefs

Create or refresh `.kit/context/workflow-briefs/` with one compact brief per
core workflow agent. These files are the local-codebase preload each workflow
agent reads before acting. They must be evidence-grounded and small enough to
avoid context bloat.

Write `workflow-explorer.md`, `workflow-implementer.md`, `workflow-reviewer.md`,
`workflow-skeptic.md`, `workflow-ui-qa.md`, and `prompt-synthesizer.md` from
`.kit/context/conventions.md`, `.kit/context/memory.md`, `.kit/context/reusables.md`,
`.wiki/*`, current code evidence, and git-archaeology output. Each brief must
stay under 5000 tokens, use bullets with `path:line` citations, avoid
implementation bodies, and replace placeholder content with real repo facts or
...
```bash
# All of these must exist, be > 200 bytes, and not contain template placeholders.
for f in .kit/context/memory.md .kit/context/conventions.md \
         .kit/context/reusables.md .wiki/index.md .wiki/architecture.md \
         .wiki/codebase.md .wiki/features.md \
         .kit/context/workflow-briefs/workflow-explorer.md \
         .kit/context/workflow-briefs/workflow-implementer.md \
         .kit/context/workflow-briefs/workflow-reviewer.md \
         .kit/context/workflow-briefs/workflow-skeptic.md \
         .kit/context/workflow-briefs/workflow-ui-qa.md \
         .kit/context/workflow-briefs/prompt-synthesizer.md; do
  if [ ! -f "$f" ] || [ $(wc -c < "$f") -lt 200 ]; then
    echo "MISSING_OR_EMPTY: $f"
  elif grep -Eq 'PLACEHOLDER|_not yet detected_|Generated by: _not yet run_' "$f"; then
    echo "PLACEHOLDER_CONTENT: $f"
  fi
done
```
...
Workflow brief gate:
- Before marking bootstrap complete, generate
  `.kit/context/workflow-briefs/{workflow-explorer,workflow-implementer,workflow-reviewer,workflow-skeptic,workflow-ui-qa,prompt-synthesizer}.md`.
- Synthesize them from `.kit/context/conventions.md`, `.kit/context/memory.md`,
  `.kit/context/reusables.md`, `.wiki/*`, current code evidence, and
  git-archaeology output.
- Each brief must stay under 5000 tokens, use bullets with `path:line`
  citations, avoid implementation bodies, and replace placeholder content with
  real repo facts or "No stable repo-specific guidance found yet."
