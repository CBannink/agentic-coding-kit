...
$SessionRoot = if ($env:AGENTS_SESSION_ROOT) { $env:AGENTS_SESSION_ROOT } else { Join-Path $AgentsRoot "session-state" }
$inspirationDir = Join-Path (Join-Path $SessionRoot $SessionId) "inspiration"
