...
#
# Targets:
#   1. Old session-state dirs (<repo>/.kit/session-state/<id>/ when bootstrapped,
#      else ~/.agents/session-state/<id>/) -- archive after age
...
#   6. Device-wide cross-repo INDEX.md -- drop rows whose handoffs no longer exist
#
...
# --- 6. Device-wide INDEX.md cleanup: drop rows pointing to missing sessions ---
...
if (Test-Path $globalIndex) {
    ...
    if ($droppedRows -gt 0 -and -not $DryRun) {
        ...
        [void]$summary.actions.Add("cleanup-index: removed $droppedRows row(s) pointing to missing handoffs")
    }
}
