...
#
# Flags:
#   -SessionId <id>           the kit session id from .kit/session-state/ in a
#                             bootstrapped repo, or ~/.agents/session-state/ fallback
...
if (-not $env:AGENTS_SESSION_ROOT) {
    $repoSessionRoot = $null
    if (Get-Command Resolve-RepoSessionRoot -ErrorAction SilentlyContinue) {
        $repoSessionRoot = Resolve-RepoSessionRoot -StartPath $RepoRoot
    }
    if ($repoSessionRoot) {
        $script:SessionRoot = $repoSessionRoot
    } elseif (-not $script:SessionRoot) {
        $script:SessionRoot = if ($script:GlobalSessionRoot) { $script:GlobalSessionRoot } else { Join-Path $HOME ".agents/session-state" }
    }
} elseif (-not $script:SessionRoot) {
