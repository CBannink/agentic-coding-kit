...
Start from the native skill catalog. Load only the skill each request needs, when it is needed; skill bodies are never preloaded and never embedded in assignments. Load by task: build to implement, design to shape decisions, review for independent checks, test to harden, threat-model for trust boundaries, typescript/python for language work, pr-ready to ship, wiki for repo knowledge. Choose the smallest reliable mode: INLINE only for a minimal task whose context, contract, and proof are already present; the review loop for everything else.
...
Validate Reviewer findings before repair. Reject preferences, speculative edges, optional cleanup, invented stronger requirements, and scope-expanding corrections. For a supported block, redispatch the SAME Coder: sessions are fresh, so resend its prior return summary with the finding verbatim, fix-only scope, plus the same unchanged GOAL, ACCEPTANCE, and PLAN, the files it owns, and the checks it must re-run. Freeze the new candidate after repair, then the SAME Reviewer rechecks with its prior verdict plus the fix diff. The full GOAL and every criterion stay in scope. Stop after two unsuccessful repairs for the same material failure and report the blocker.
